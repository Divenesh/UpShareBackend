...
{
    public static List<Dictionary<string, object>> GetInfoForCategories(
    ...
    public static List<Dictionary<string, object>> GetInfoForItemById(
        List<Dictionary<string, object>> items,
        Guid guid
    )
    {
        try
        {
            ...
            var sellerId =
                itemList.Count > 0
                    ? itemList[0]?["sellerId"]?.ToString() ?? string.Empty
                    : string.Empty;
            ...
            if (itemList.Count > 0)
            {
                var itemCollection = new Dictionary<string, object> { ["item"] = itemList[0] };
                result.Add(itemCollection);
            }
            if (seller.Count > 0)
            {
                var sellerCollection = new Dictionary<string, object> { ["sellers"] = seller };
            ...
            if (rating.Count > 0)
            {
                var ratingCollection = new Dictionary<string, object> { ["ratings"] = rating };
    ...
    public static async Task<Dictionary<string, object>> UpdateUser(object user)
    {
        try
        {
            var result = await apiPaths.UpdateUser(user);
            return result ?? new Dictionary<string, object>();
        }
        catch (Exception ex)
        {
